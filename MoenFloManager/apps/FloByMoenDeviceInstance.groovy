...
def initialize() {
  ...
  log.debug "${locationName} - ${deviceInfo?.nickname}"
	app.updateLabel("${locationName} - ${deviceInfo?.nickname}")
...
def createDevice() {
  ...
  def childDevice = getChildDevice("${deviceId}-${appId}")
  ...
  if (!childDevice) {
    if (device) {
      ...
      log.error("Unable to find device info. Device creation failed.")
    }
  }
